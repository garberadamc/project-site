...
## Step 0: Preparing to work with MplusAutomation:
...
## Step 2: Installing & loading packages
...
```{r}
# the folllowing 3 lines install the “rhdf5” package to read gh5 files
...
## Step 4: A couple ways to explore & view data in R
...
###  Step 5: View dataframe with labels & response scale meta-data

#### Note: using the "print" option you can save a PDF as a codebook of metadata
...
## Step 6: Writing / converting data between 3 formats
...
### Step 7: Preparing data column-names to be `MplusAutomation` ready

#### how to make all variable names fit within the 8-character name limit (Mplus) while avoiding duplicates?
...
## Step 8: Filtering rows (observations) & selecting columns (variables).

## ______________________________________________

#### When to use: This allows you to make dataframe subsets for particular analyses or other purposes
...
## Step 9: changing variable class types (e.g., class `numeric` to `factor` )
...
## Step 10: visualizing data using `ggplot()`
...
#### In factor analysis you often want to look at a group of variables at once: 
...
### Step 11: correlations - there are many ways to visualize bi-variate relations in R

#### using packages: {`corrplot`}, {`corrgram`}, or {`ggpairs`}
...
### Step 12: simple introduction to `MplusAutomation` (writing, running, & reading models)
...
#### PRACTICE 01: Using MplusObject() method (type = BASIC;)
```{r}
...
#### Now exploring descriptives for observations that reported as "female" 

#### Line of syntax added: "useobs = female == 1;"
...
#### Exploratory Factor Analysis (EFA) Lab Demonstration
...
## END OF LAB - Explore, prepare data, & an introduction to MplusAutomation
